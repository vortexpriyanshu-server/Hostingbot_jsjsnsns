...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            types.InlineKeyboardButton('💳 Subscriptions', callback_data='subscription', style='success'), #0
...
def create_reply_keyboard_main_menu(user_id):
    ...
    for row_buttons_text in layout_to_use:
        markup.add(*[types.KeyboardButton(text, style=('danger' if text == '🔒 Lock Bot' else 'success' if text == '🟢 Running All Code' else 'primary')) for text in row_buttons_text])
    return markup
